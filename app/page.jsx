...
Format the output with clear section headers. Be specific and clinical.`;
  ...
  const sendSoapEmail = async () => {
    ...
    try {
      const subject = encodeURIComponent(\`SOAP Note — Virtual Consultation — \${patientName}\`);
      const emailBody = encodeURIComponent(\`Dr. Fleshner,\n\nPlease find the SOAP note from the virtual consultation with \${patientName} below.\n\n\${"═".repeat(60)}\n\n\${soapNote}\n\n\${"═".repeat(60)}\n\nGenerated by AskDrFleshner Virtual Consultation Platform\nThis is a demonstration — not for clinical use.\`);
      window.open(\`mailto:\${doctorEmail}?subject=\${subject}&body=\${emailBody}\`, "_blank");
      setEmailSent(true);
    } catch (err) {
