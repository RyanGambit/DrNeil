...
export async function upsertTester({ firstName, lastName, role, mode }) {
  if (!isKVConfigured()) {
    return { tester: { id: null, firstName, lastName, role, mode: mode || null, firstSeenAt: null, lastSeenAt: null }, completedScenarios: [] };
  }
  ...
  // Normalize mode — keep both "tester" (clinician evaluating) and
  // "patient" (the original patient flow) in the same store so admin
  // can see everyone. Default to "tester" for backward compatibility
  // (old records before mode was tracked).
  const normalizedMode = mode === "patient" ? "patient" : "tester";
  ...
  if (!testerId) {
    ...
    // Update lastSeenAt, role, and mode (mode upgrades from patient→tester
    // are allowed if the same person ever switches; downgrades are kept).
    tester = {
      ...tester,
      lastSeenAt: now,
      role: role || tester.role || null,
      mode: normalizedMode || tester.mode || "tester",
    };
    await kv.set(`tester:${testerId}`, tester);
  }
...
export async function startSession({ testerId, scenarioId, condition }) {
  if (!isKVConfigured()) {
    return { id: null, testerId, scenarioId: scenarioId || null, condition: condition || null, startedAt: Date.now(), completedAt: null, turns: 0, finalOutcome: null };
  }
...
export async function completeSession({ sessionId, turns, finalOutcome }) {
  if (!isKVConfigured()) {
    return { id: sessionId, completedAt: Date.now(), turns: turns || 0, finalOutcome: finalOutcome || null };
  }
